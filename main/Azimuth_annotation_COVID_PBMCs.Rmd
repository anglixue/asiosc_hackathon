...
library(dplyr)
library(ggplot2)
source("~/Documents/Study/Post-doc/project/ASI_OZS_Hackathon/asiosc_hackathon/src/recode.R")
```
...
```{r pbmc_dimplot_check, fig.width = 12, fig.height = 5, eval = FALSE}
...
```{r data_explore, results='hide', eval = FALSE}
...
```{r RNA_assay, results='hide', eval = FALSE}
...
```{r run_Azimuth_by_RNA-seq, results='hide', eval = FALSE}
...
```{r pbmc_dimplot_RNA, fig.width=12, fig.height=5, eval = FALSE}
# data1 <- readRDS("../results/innate_adapative_rna_processed.rds")
p1 <- DimPlot(data1, group.by = "cell_type", label = TRUE, label.size = 3) + NoLegend() + ggtitle("Previous RNA annotation")
p2 <- DimPlot(data1, group.by = "cell_type", label = TRUE, label.size = 3) + NoLegend() + ggtitle("Current RNA annotation")
p1 + p2
```

Question: can we use the ADT assay to annotate the cell type?

```{r ADT_assay, results='hide', eval = FALSE}
...
```{r run_Azimuth_by_CITE-seq, results='hide', eval = FALSE}
# The RunAzimuth function can take a Seurat object as input
...
```{r pbmc_dimplot_ADT, fig.width=12, fig.height=5, eval = FALSE}
...
```{r pbmc_dimplot_compare, fig.width=12, fig.height=5, eval = FALSE}
...
```{r pbmc_res1, fig.width = 12, fig.height = 10, eval = FALSE}
